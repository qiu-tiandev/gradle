...
public final class AttributeTypeValidator {
     ...
     * <p>
     * Runs at most once per {@code ruleClass}. If the type argument cannot be resolved to a
     * concrete class (rule is itself generic, uses a wildcard, or hides its type argument
     * behind an abstract intermediate whose interface uses an unbound variable), then the check
     * silently accepts as there is no reliable value to validate.
     *
     * @throws IllegalAttributeTypeException if the type argument is resolved to a class that is not
     *         one of the supported attribute value types.
     */
    public static void validateRuleTypeParameter(Class<?> ruleClass, Class<?> ruleInterface) {
        ...
        if (typeArg != null && !isSupportedAttributeType(typeArg)) {
            throw new IllegalAttributeTypeException(
                String.format(
                    "Unsupported type '%s' declared as the type parameter of attribute rule '%s'. Attribute values must be of type String, Boolean, a subtype of Number, or implement %s.",
                    typeArg.getName(), ruleClass.getName(), Named.class.getName()
                ),
                typeArg
            );
        }

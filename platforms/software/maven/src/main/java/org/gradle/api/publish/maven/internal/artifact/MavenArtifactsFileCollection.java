...
class MavenArtifactsFileCollection extends AbstractFileCollection implements PublicationArtifactSetFileCollection {
    ...
    MavenArtifactsFileCollection(String publicationName, Iterable<MavenArtifact> artifacts, TaskDependencyFactory taskDependencyFactory) {
    ...
    private static ProviderInternal<File> classify(MavenArtifact artifact) {
        if (artifact instanceof PublishArtifactBasedMavenArtifact) {
            PublishArtifact inner = ((PublishArtifactBasedMavenArtifact) artifact).getPublishArtifact();
